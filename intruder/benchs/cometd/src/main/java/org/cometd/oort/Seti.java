        ...
        if (message!=null && toUid!=null)
        {
            ...
            if (location==null && _setiChannelId.equals(msg.getChannel())) {
             // was sent to this node, so escalate to the shard.
                ...
                // 其实这里什么都没做  ShardLocation中的receive是空的
                Log.info("ShardLocation debug: user id is " + toUid + " location is " + location);
            } else if (location == null) {
                Log.info("ShardLocation debug: location is null " + toUid + " location is " + location);
            }
