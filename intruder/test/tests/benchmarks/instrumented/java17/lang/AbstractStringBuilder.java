...
package benchmarks.instrumented.java17.lang;
...
abstract class AbstractStringBuilder implements Appendable, CharSequence {
    ...
    static int codePointCountImpl(char[] a, int offset, int count) {
        ...
        for (int i = offset; i < endIndex; ) {
            if (Character.isHighSurrogate(a[i++]) && i < endIndex &&
                Character.isLowSurrogate(a[i])) {
                n--;
                i++;
            }
        }
    ...
    static int offsetByCodePointsImpl(char[]a, int start, int count,
        ...
        if (codePointOffset >= 0) {
            ...
            for (i = 0; x < limit && i < codePointOffset; i++) {
                if (Character.isHighSurrogate(a[x++]) && x < limit &&
                    Character.isLowSurrogate(a[x])) {
                    x++;
                }
            }
            ...
            for (i = codePointOffset; x > start && i < 0; i++) {
                if (Character.isLowSurrogate(a[--x]) && x > start &&
                    Character.isHighSurrogate(a[x-1])) {
                    x--;
                }
            }
            if (i < 0) {
